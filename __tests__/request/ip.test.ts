...
import {
  ...
  isTestRequest,
  isTestSocket,
} from "../../test-helpers/typed-helpers";
...
describe("req.ip", () => {
  describe("with req.ips present", () => {
    it("should return req.ips[0]", () => {
      ...
      if (!isTestSocket(socket)) {
        throw new Error("Socket is not a valid TestSocket");
      }

      const req = { headers: {}, socket };
      if (!hasRequestProps(req)) {
      ...
      if (!isTestRequest(request)) {
        throw new Error("Request is not a valid ExtendedRequest");
      }
